...
public class LoadBalancer {
    private static void startLoadBalancer(String schedAlgo) {
        try {
            ...
            // int currentWorker = portNo % workers.size();
            int currentWorker = -1;
            int i=0;
            ...
            while (!Thread.interrupted()) {
                ...
                if (schedAlgo.equals("RR")) {
                    int idx = i % workers.size();
                     int port = workers.get(idx).getPort();
                    String host = workers.get(idx).getHost();
                    int hash_host;
                    hash_host =host.hashCode();
                    System.out.println("address : " + clientSocket.getLocalAddress().toString());
                    System.out.println("hash of Host is" + hash_host);
                    currentWorker = (hash_host) % workers.size();
                   System.out.println("port is" + port+ "worker is"+ currentWorker);
		...
		//Socket workerSocket = new Socket("localhost",20004);
		System.out.println("Completed creating socket" +workers.get(currentWorker).getPort());
